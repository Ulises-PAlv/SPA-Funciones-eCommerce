...
export class ProductsService {
  ...
  constructor() {
    console.log('ProductsService Works');
  }
  ...
  public searchProducts(strHTML) {
    let results: any = [];

    strHTML = strHTML.toLowerCase();

    for(const product of this.Products) {
      const modelAux = product.model.toLowerCase();
      const brandAux = product.brand.toLowerCase();

      if( (modelAux.indexOf(strHTML) !== -1) || (brandAux.indexOf(strHTML) !== -1)) {
  ...
  public getDepartmentArray(nameDep) {
    let arrayDep: any = [];

    for(const product of this.Products) {

      if(product.department === nameDep) {
        arrayDep.push(product);
      }
    }
